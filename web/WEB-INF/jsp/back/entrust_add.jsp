...
<script type="text/javascript">
	...
	if(msg == "success"){//保存成功的回传
		parent.$("#entrustlist_table").datagrid('reload');
		parent.$.messager.alert("提示", "保存成功");
		parent.closeWindow("#w1");
	}
	$(function() {
        $("#entrust_save").click(function() {
	        if($("#entrust_addform").form('validate')){
	        	$("#entrust_addform").submit();
	        }
		});
	});
</script>

<form id="entrust_addform" style="margin: 5px 5px;" method="post"  action="entrustSave">
	<a href="javascript:;" class="easyui-linkbutton" iconCls="icon-ok" id="entrust_save">已处理</a>
	...
	<div class="sztab">
		<table id="entrust_addtable">
			<tr>
				<td>委托标的名</td>
				<td>
				   <input id="id" type="hidden" name="id" value="${entrust.id }"/>
				   <input id="title" name="title" class="easyui-textbox" data-options="required:true,readonly:true" value="${entrust.title }" />
				</td>
			</tr>
			<tr>
				<td>委托人</td>
				<td>
				 <input id="name"  name="name" class="easyui-textbox" data-options="required:true,readonly:true"  value="${entrust.name }"/>
				</td>
			</tr>
			<tr>
				<td>委托人电话</td>
				<td>
				 <input id="tel" name="tel" class="easyui-numberbox" data-options="required:true,readonly:true" value="${entrust.tel }" />
				</td>
			</tr>
			<tr>
				<td>描述</td>
				<td style="text-align: left">
					<textarea name="content" cols="100" rows="8" style="width:95%;height:350px;visibility:hidden;">${entrust.content }</textarea>
				</td>
			</tr>
		</table>
	</div>
