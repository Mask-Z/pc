...
<form id="rzdk_addform" style="margin: 5px 5px;" method="post"  action="onlineLoansSave">
	...
	<div class="sztab">
		<table id="zxdk_addtable">
			<%--<tr>--%>
				<%--<td>序号</td>--%>
				<%--<td>--%>
				   <input id="id" type="hidden" name="id" value="${zxdkApply.id }"/>
				<%--</td>--%>
			<%--</tr>--%>
			<tr>
				<td>状态</td>
				<td>
					<select name="status">
						<option value="0" <c:if test="${zxdkApply.status.equals('0')}">selected</c:if> >待联系</option>
					    <option value="1" <c:if test="${zxdkApply.status.equals('1')}">selected</c:if> >已联系</option>
					    <option value="2" <c:if test="${zxdkApply.status.equals('2')}">selected</c:if> >完成</option>
					</select>
				</td>
			</tr>
			...
			<tr>
				...
				<td style="text-align: left">
					<textarea name="remark" cols="100" rows="8" style="width:95%;height:350px;visibility:hidden;">${zxdkApply.remark }</textarea>
				</td>
			</tr>
		</table>
	</div>
