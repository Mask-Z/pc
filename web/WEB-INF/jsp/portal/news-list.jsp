          ...
          <div class="smc clearfix">
              <ul class="clearfix">
	              <c:forEach var="news" items="${newsArray}">
	              	<li>
	              		<a href="<%=basePath%>newsinfo?infoid=${news.infoid}">
	              		  <span class="fr mr5 f12 fc99">${news.infodate }</span>${news.title }
	              		</a>
	                </li>
				  </c:forEach>
              </ul>
  ...
  <div id="kkpager" class="kkpager" style="display: block;">
    <div class="clearfix">
      <span class="pageBtnWrap">
	      <c:if test="${page>1}">
	      	 <a href="<%=basePath %>news_more?catagoryNum=${catagoryNum}&page=${page-1}" title="上一页">上一页</a>
		  </c:if>
		  <c:if test="${page-3 gt 0}">
		  	<a href="<%=basePath %>news_more?catagoryNum=${catagoryNum}&page=${page-3}" >${page-3}</a>
		  </c:if>
		  <c:if test="${page-2 gt 0}">
		  	<a href="<%=basePath %>news_more?catagoryNum=${catagoryNum}&page=${page-2}" >${page-2}</a>
		  </c:if>
		  <c:if test="${page-1 gt 0}">
		  	<a href="<%=basePath %>news_more?catagoryNum=${catagoryNum}&page=${page-1}" >${page-1}</a>
		  </c:if>
        ...
        <c:if test="${page<totalPage}">
	      	 <a href="<%=basePath %>news_more?catagoryNum=${catagoryNum}&page=${page+1}" title="下一页">下一页</a></span>
		</c:if>
