...
<body>
       ...
       <div class="crumbs clearfix">
        <div class="fl">
            ...
            &gt; <a href="<%=basePath%>jjz_more?danWeiGuid=${danWeiGuid}" target="_blank">竞价中</a>
          </div>
      </div>
        </div>
   ...
   <div class="enroll-list-box">
      <ul class="clearfix">
          <c:forEach items="${list }" var="jjz">
		      <li>
		          <c:if test="${jjz.projectType=='pt' }">
		          <div class="pic">
		              <a href="infodetail?infoid=${jjz.infoid }&categoryNum=${jjz.categorynum }" target="_blank" class="content4-detail"><img src="${jjz.titlepic }" alt="" class="p"></a>
                  </div>                  
                  <div class="title"><a href="infodetail?infoid=${jjz.infoid }&categoryNum=${jjz.categorynum }" target="_blank" class="content4-detail"><b>[${jjz.areaName }]</b>${jjz.title }</a></div>
                  <div class="item">
                      <c:if test="${jjz.systemtype != 'ZZKG' }"> 
                          挂牌价：<span class="fcdd3"><span class="bold">${jjz.guapaiprice }</span><c:if test="${jjz.guapaiprice ne '面议'}"> 元</c:if></span>
                      </c:if>
                      <c:if test="${jjz.systemtype == 'ZZKG' }"> 
                          挂牌价：<span class="fcdd3"><span class="bold">${jjz.zzkg_guapaiprice }</span></span>
                      </c:if>                                                        
                  </div>
                  <div class="item">
                                                                  竞价结束时间：${jjz.jingjia_end }
                  </div>
                  </c:if>
                  <c:if test="${jjz.projectType=='zt' }">
                  <div class="pic">
                      <a href="javascript:showOrHide('${jjz.ProjectGuid }');"><img src="image/default/vip.jpg" alt="" class="p"></a>
                  </div>  
                  <div class="title"><a href="javascript:showOrHide('${jjz.ProjectGuid }');"><b>[专厅项目]</b>${jjz.title }</a></div>                                  
                  <input type="hidden" id="jjz_flag_${jjz.ProjectGuid }" value="0"/>       
                  <c:forEach items="${jjz.listGg }" var="gg">
                      <div class="title jjz_zt_${jjz.ProjectGuid }" style="display: none"><a href="infodetail?infoid=${gg.infoid }&categoryNum=${gg.categorynum }" target="_blank" class="content4-detail"><b>[${gg.areaName }]</b>${gg.title }</a></div>
                  </c:forEach>
                  </c:if>
                  <div class="item">
                      <a href="javascript:void(0)" class="state-btn fr btn-update">${jjz.isZiShenPassCn }</a>
                      <a href="javascript:void(0)" class="state-btn fr btn-bidding mr5">${jjz.bZJIsjiaoNa }</a>                                                                 
                  </div>
              </li>
		  </c:forEach>                       
      </ul>
...
function queryData(page){
	...
	var url=basePath+"jjz_more?danWeiGuid="+danWeiGuid+"&time="+new Date().getTime();
	if(page!=null){
		url+='&page='+page;
	}
...
function showOrHide(zhuanTingGuid){
    var flag = $("#jjz_flag_"+zhuanTingGuid).val();
    $(".jjz_zt_"+zhuanTingGuid).each(function(){
        if(flag=="0"){
            $("#jjz_flag_"+zhuanTingGuid).val("1");
            $(this).show();
        }else{
            $("#jjz_flag_"+zhuanTingGuid).val("0");
            $(this).hide();
        }			    
    });
}
