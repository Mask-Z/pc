...
<head>
    ...
    <script src="<%=basePath%>js/zzcg/list.js"></script>
	<style type="text/css">
		dd{
			cursor: pointer;
		}
...
<body>
       ...
       <div class="crumbs clearfix">
        <div class="fl">
            ...
            <a href="<%=basePath%>zzcg_more">自主采购</a>&gt;
          </div>
    ...
    <div class="list-select-search">
        <ul class="select clearfix">
            ...
            <li class="select-list">
              <dl id="select2">
                <dt>公告类型：</dt>
                <dd  class="select-all selected"><a  href="javascript:;">采购公告</a></dd>
                       ...
                       <c:when test="${type eq 'cggg'}">
                           <ul class="clearfix five">
                               ...
                               <c:forEach items="${rows }" var="info">
                                   <li>
                                       <div class=" title"><a
                                               href="<%=basePath %>zzcg_detail?tag=cggg&id=${info.id}"
                                               target="_blank" title="${info.title }">${info.title }</a>
                                       </div>
                                       <div class="item">
                                           <p class="unit" title="${info.dwname }">${info.dwname }</p>
                                       </div>
                                       <div class="time">${fn:substring(info.sign_end_date,0,19) }</div>
                                       <div class="info2">${info.hitcount }</div>
                                       <c:if test="${info.status_name == '1'}" >
									   		<div class="info2"><a  class="state-btn  btn-update">未开始</a></div>
									   </c:if>
									   <c:if test="${info.status_name == '2'}" >
									   		<div class="info2"><a onclick="window.open('http://58.216.221.107:20017/eip')" class="state-btn  btn-enroll">报名中</a></div>
									   </c:if>
									   <c:if test="${info.status_name == '3'}" >
                       ...
                       <c:otherwise>
                           <ul class="clearfix ">
                               ...
                               <c:forEach items="${rows }" var="info">
                                   <li>
                                       <div class=" title"><a
                                               href="<%=basePath %>zzcg_detail?tag=cjgg&id=${info.id}"
                                               target="_blank" title="${info.title }">${info.title }</a>
                                       </div>
                                       <div class="item">
                                           <p class="unit" title="${info.xiaquname }">${info.dwname }</p>
                                       </div>
                                       <div class="time">${fn:substring(info.sign_end_date,0,19) }</div>
                                   </li>
                               </c:forEach>
