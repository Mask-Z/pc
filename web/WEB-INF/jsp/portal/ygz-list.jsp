...
<body>
       ...
       <div class="crumbs clearfix">
        <div class="fl">
            ...
            &gt; <a href="<%=basePath%>ygz_more?danWeiGuid=${danWeiGuid}" target="_blank">已关注</a>
          </div>
      </div>
        </div>
 ...
 <div class="enroll-list-box">
      <ul class="clearfix">
          <c:forEach items="${list }" var="ygz">
		      <li>
		          <div class="pic">
		              <a href="infodetail?infoid=${ygz.infoid }&categoryNum=${ygz.categorynum }" target="_blank" class="content4-detail"><img src="${ygz.titlepic }" alt="" class="p"></a>
                  </div>
                  <div class="title"><a href="infodetail?infoid=${ygz.infoid }&categoryNum=${ygz.categorynum }" target="_blank" class="content4-detail"><b>[${ygz.areaName }]</b>${ygz.title }</a></div>
                  <div class="item">
                      <c:if test="${ygz.systemtype != 'ZZKG' }"> 
                          挂牌价：<span class="fcdd3"><span class="bold">${ygz.guapaiprice }</span><c:if test="${ygz.guapaiprice ne '面议'}"> 元</c:if></span>
                      </c:if>
                      <c:if test="${ygz.systemtype == 'ZZKG' }"> 
                          挂牌价：<span class="fcdd3"><span class="bold">${ygz.zzkg_guapaiprice }</span></span>
                      </c:if>
                  </div>
                  <div class="item">
                                                                  报名截止时间：${ygz.gonggaotodate_str }
                  </div>
              </li>
		  </c:forEach>                       
      </ul>
...
function queryData(page){
	...
	var url=basePath+"ygz_more?danWeiGuid="+danWeiGuid+"&time="+new Date().getTime();
	if(page!=null){
		url+='&page='+page;
	}
